...
服务端固定预留底部 **36 像素纯白区域**，在标准 `1072×1448` 图片上对应 `x=0..1071, y=1412..1447`。日程框与该区域之间留出间隔。客户端可在图片显示后用约 22–26 像素字号叠画电量/充电等本地状态；服务端不生成或缓存设备状态。当前客户端尚未实现该叠画逻辑，每次整图显示会清除之前的叠画，客户端实现时需重新绘制。本次只改变服务端排版，PNG 尺寸、灰度格式及下载协议不变。


## 可选：动态电量图片接口

`status_api.py` 从固定本地日程 PNG 读取底图，接收带 Bearer 认证的
`POST /image`，将电量/充电状态绘制到末尾 36 像素并直接返回 PNG。
原静态图片、上传与分享流程不变；电量缺失、越界或 JSON 无效时仍出图，
显示电量未知；字体或底栏绘制失败则返回原底图像素。认证、协议大小与
超时保护不降级，底图不可用时返回错误。Kindle 客户端接入不包含在本模块。

只监听 loopback，需要另行部署 HTTPS 反向代理。密钥必须通过所有者持有的
0600 文件提供，不能放在 URL、命令行值、日志或仓库中。
详见 [接口契约、启动参数、安全边界与测试](docs/status-api.md)。
