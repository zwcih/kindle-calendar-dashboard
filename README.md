...
**本地状态栏**：图片提供方须在 **1072×1448 源图底部预留36像素白栏**；客户端跟随实际整图显示，将电量和 `Updated` 内容时间叠加到这条白栏缩放后的区域。每次实际显示前只读查询 FBInk 可见宽高，统一换算清理、图标、文字和刷新坐标；源图尺寸、虚拟 framebuffer／stride 都不当作物理屏幕尺寸。**本次不修改服务端生成器**，未留白图片仍可能被覆盖内容。电量仅表示上次实际显示时的采样，不是实时监控；不增加定时器、网络接口或唤醒。**专用模式图片 SHA 未变时不刷新任何区域，也不额外查询几何或采样电量**，保留原有省电跳过行为。
...
**加入状态栏之前的版本**已实测独立手动下载／显示／缓存、专用模式启动与再次启动、真实休眠、单指刷新和长按退出的完整恢复；独立并发、严格 SIGKILL 与恢复套件已在 Linux `/bin/sh=dash` 全部通过。状态栏曾因把源图尺寸硬编码为屏幕尺寸而在末次更新得到 `MXCFB_SEND_UPDATE: Invalid argument`，随后正常恢复 Home。**当前已改为实际可见几何驱动全部绘制，但修正版尚待真机验收**，不能用模拟／CI代替设备通过。限量私有错误诊断保留，预算及恢复流程不变。完整设备崩溃套件、物理多指序列、新版半小时自动时隙及整夜运行仍无新增验收。详见[客户端依赖、配置与排障](kindle/README.md)。
